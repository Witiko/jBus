...
All functionality of the library is embedded in the `JBus` object with the `JBus.Node` object, its methods and the `JBus.Scope` method being all a jBus application should ever need. The `JBus.services` and `JBus.messages` objects are only present for debugging purposes.
